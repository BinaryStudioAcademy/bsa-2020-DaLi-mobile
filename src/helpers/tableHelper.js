...
const sortRows = (rows, sortOrder, sortOrderBy) =>
...
export const formatDateForSummarize = (data, config) => {
  if (config?.isSummarize && config?.summarize.groupBy.type === 'date') {
    ...
    return data.map((row) => {
      const newRow = {...row};
      newRow[dateColumn] = getDate(
        newRow[dateColumn],
        config.summarize.groupBy.period,
      );
      return newRow;
    });
  }
...
export const getRowsPages = (rows, itemsPerPage) => {
  let rowsPages = [];
  for (var i = 0; i < rows.length; i += itemsPerPage) {
    rowsPages.push(rows.slice(i, i + itemsPerPage));
  }
  return rowsPages;
};
